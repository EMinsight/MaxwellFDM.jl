...
smooth_param!(paramKd, oindKd′::AbsArr{ObjInd}, oind2shp, oind2pind, pind2matprm, isfield˔shp, gt₀, l, l′, σ, ∆τ′) =
    smooth_param!(paramKd, tuple(oindKd′), oind2shp, oind2pind, pind2matprm, isfield˔shp, gt₀, l, l′, σ, ∆τ′)
                ...
                if Nprm_vxl ≥ 2
                    if Nprm_vxl == 2
                        ...
                        prm_vxl = smooth_param_vxl(ci_vxl′, ind_c, n_diffp, pind_vxl′, oind_vxl′,
                                                   oind_cmp′, oind2shp, oind2pind, pind2matprm, isfield˔shp,
                                                   lcmp, lcmp′, σcmp, ∆τcmp′)
                    else  # Nprm_vxl ≥ 3
...
function smooth_param_vxl(ci_vxl′::Tuple2{CartesianIndex{K}},
                          ...
                          oind2pind::AbsVec{ParamInd},  # input map from oind to pind
                          pind2matprm::AbsVec{SSComplex{Kp,Kp²}},  # map from pind to electric (magnetic) material parameters; Kp² = Kp^2
                          isfield˔shp::Bool,  # true if spaces where material parameters and shapes are defined are orthogonal complement of each other; false if they are the same
