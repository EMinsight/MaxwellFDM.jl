...
function smooth_param!(paramKd::AbsArrComplex{K₊₂},  # parameter array to smooth at voxel centers
                       ...
                       ismatprm˔shp::Bool,  # true if spaces where material parameters and shapes are defined are orthogonal complement of each other; false if they are the same
                ...
                if Nprm_vxl ≥ 2
                    if Nprm_vxl == 2
                        ...
                        prm_vxl = smooth_param_vxl(ci_vxl′, ind_c, n_diffp, pind_vxl′, oind_vxl′,
                                                   oind_cmp′, oind2shp, oind2pind, pind2matprm, ismatprm˔shp,
                                                   lcmp, lcmp′, σcmp, ∆τcmp′)
                    else  # Nprm_vxl ≥ 3
...
function smooth_param_vxl(ci_vxl′::Tuple2{CartesianIndex{K}},
        ...
        if iszero(nout)
            ...
            prm_vxl = ismatprm˔shp ? kottke_avg_param(prm_fg, prm_bg, rvol) :  # field is always parallel to shape boundaries
