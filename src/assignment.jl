...
# surface normal is correctly calculated at the points on periodic boundaries.  This is
# achieved by making the object to extrude the periodic boundary sufficiently deep with the
# shape appearing on the opposite periodic boundary.  If the object extrudes the periodic
# boundary too shallow, with the extruded part is cut with a plane parallel to the domain
# boundary, then some boundary points within the object could be closer to the cut surface
# than the actual object surface, such that the boundary normal could be wrongly chosen as
# the surface normal.  It is safe, and also easy, to make the object to extrude a periodic
# boundary by the entire shape appearing on the opposite boundary, because then the boundary
# points cannot feel the effect of the termination of the object by periodic boundaries at
# all when calculating the surface normal.  This means that when mimicking a 2D problem by a
# 3D problem with one cell in the z-direction, putting prism-shaped objects with prism
# height of ∆z is not sufficient; it is safer to make the prism height 3∆z, as on the
# +z-boundary the object appearing on the opposite boundary has a thickness of ∆z, and on
# the -z-boundary the object appearing on the opposite boundary has also a thickness of ∆z.
# See example/usage_uni.jl.)
#
...
# Unlike smoothing.jl/smooth_param! and param.jl/create_paramop, the paramKd and oindKd′
