...
# About Kp⏐₁
#
# The number of entries in the tuple oindKd′ is Kp⏐₁, which is Kp or 1.  It is the parameter
# used to control the assignment behavior:
# - For Kp⏐₁ == 1, the material parameter tensor is evaluated at the location specified by gt₀
# in each Yee cell, and the off-diagonal entries are assigned.
# - For Kp⏐₁ == Kp, the material parameter tensor is evaluated at the half-index shifted
# locations from gt₀ in each Yee cell, and at each half-index shifted location the
# corresponding diagonal entry is assigned.
#
# If Kp == 1, it is ambiguous which of the two assignment behaviors is taken, because Kp⏐₁ is
# 1 and Kp simultaneously.  In fact, a mixed behavior is chosen: the choice of the location
# to evaluate the material parameter tensor follows the behavior for Kp⏐₁ == 1 (i.e., does
# not shift from the location specified by gt₀), but the choice of tensor entries to assign
# follows the behavior for Kp⏐₁ == Kp (i.e., assign the diagonal entries, which actually
...
assign_param!(paramKd, oindKd′::AbsArr{ObjInd}, gt₀, oind2shp, oind2pind, pind2matprm, τl, isbloch) =
    assign_param!(paramKd, tuple(oindKd′), gt₀, oind2shp, oind2pind, pind2matprm, τl, isbloch)
...
function assign_param!(paramKd::AbsArrComplex{K₊₂},  # electric (magnetic) parameter array to set; K₊₂ = K+2, where 2 is rank of material tensor
                       oindKd′::NTuple{Kp⏐₁,AbsArr{ObjInd,K}},  # object index arrays to set at electric (magnetic) parameter locations; Kp⏐₁ = Kp or 1
    ...
    # Perform assignment.
    for nw = 1:Kp⏐₁  # if Kp⏐₁ == Kp for Kp = 3, w = X̂, Ŷ, Ẑ
...
# For Kp⏐₁ == 1.
# Below, note that the element type of pint2matprm, SSComplex{Kp,Kp²}, is always a matrix
...
# of the value of Kp, where 2 is the rank of the material parameter tensor.
...
function assign_val!(array::AbsArr{T,K₊₂}, tensor::SMatrix{Kp,Kp,T}, CI::CartesianIndices{K}) where {T,K,Kp,K₊₂}  # K₊₂ = K+2
    # Below the main diagonal.
    for c = 1:Kp, r = c+1:Kp, ci = CI  # column- and row-indices and location index
        @inbounds array[ci,r,c] = tensor[r,c]
    end

    # Above the main diagonal.
    for c = 2:Kp, r = 1:c-1, ci = CI  # column- and row-indices and location index
        @inbounds array[ci,r,c] = tensor[r,c]
    end
