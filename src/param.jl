...
function create_paramop(paramKd::AbsArrComplex{K₊₂},  # material parameter array
    ...
    kdiag = 0
    paramop = create_paramop_supdiag(paramKd, kdiag, N, order_cmpfirst=order_cmpfirst)  # diagonal components of ε tensor
    for kdiag = 1:Kf-1  # index of diagonal of ε tensor
        ...
        # Below, Min and Mout are block-diagonal, but paramopₖ is block-off-diagonal
        # (1 ≤ kdiag ≤ Kf-1).  See my bullet point entitled [Update (May/13/2018)] in RN -
        # Subpixel Smoothing.
        paramopₖ = create_paramop_supdiag(paramKd, kdiag, N, order_cmpfirst=order_cmpfirst)
        paramop += Mout * paramopₖ * Min
    end
