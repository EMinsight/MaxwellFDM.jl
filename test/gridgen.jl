...
@testset "gen_lprim1d for fixed intervals to fail" begin
    ...
    sublprim = MaxwellFD3D.gen_sublprim1d(domain, PRIM, intvprim, intvdual, lprim0, ldual0)
    # info("sublprim = $sublprim")
    @test_throws ErrorException MaxwellFD3D.gen_lprim1d(domain, PRIM, intvprim, intvdual, lprim0, ldual0, rt, rmax)
...
@testset "gen_lprim1d for fixed intervals" begin
    ...
    # info("rt = $rt")
    # info("rmax = $rmax")
    lprim = MaxwellFD3D.gen_lprim1d(domain, PRIM, intvprim, intvdual, lprim0, ldual0, rt, rmax)
    # info("lprim = $lprim")
    @test all(lprim[[1,end]] .== bounds(domain))
    @test MaxwellFD3D.issmooth(diff(lprim), rmax)
...
@testset "gen_lprim1d for fixed intervals, dual domain" begin
    ...
    sublprim = MaxwellFD3D.gen_sublprim1d(domain, DUAL, intvprim, intvdual, lprim0, ldual0)
    # info("sublprim = $sublprim")
    @test typeof(sublprim) == Vector{Vector{Float64}}
    @test isodd(length(sublprim))
    subgrid = sublprim[1:2:end]
    ...
    # info("rt = $rt")
    # info("rmax = $rmax")
    lprim = MaxwellFD3D.gen_lprim1d(domain, DUAL, intvprim, intvdual, lprim0, ldual0, rt, rmax)
    # info("lprim = $lprim")
...
@testset "comp_lprim1d" begin
    ...
    lprim = MaxwellFD3D.comp_lprim1d(sublprim, rt, rmax)
    @test MaxwellFD3D.issmooth(diff(lprim), rmax)
...
@testset "gen_lprim" begin
    ...
    @test MaxwellFD3D.issmooth(diff(xprim), rmax)
    @test MaxwellFD3D.issmooth(diff(yprim), rmax)
    @test MaxwellFD3D.issmooth(diff(zprim), rmax)
end
