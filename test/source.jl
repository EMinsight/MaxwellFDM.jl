...
@testset "distweights" begin
    ...
    c = 8  # at grid point
    @test all(MaxwellWave.distweights(c, PRIM, domain, lprim, ∆lprim, true) .≈ ([3,3], [1/∆lprim[3], 0]))
    @test all(MaxwellWave.distweights(c, PRIM, domain, lprim, ∆lprim, false) .≈ ([3,3], [1/∆lprim[3], 0]))

    c = 11  # between grid points
    r = (c-8) / (15-8)
    @test all(MaxwellWave.distweights(c, PRIM, domain, lprim, ∆lprim, true) .≈ ([3,4], [1/∆lprim[3]*(1-r), 1/∆lprim[4]*r]))
    @test all(MaxwellWave.distweights(c, PRIM, domain, lprim, ∆lprim, false) .≈ ([3,4], [1/∆lprim[3]*(1-r), 1/∆lprim[4]*r]))

    c = 3  # farthest point affected by negative-end boundary
    @test all(MaxwellWave.distweights(c, PRIM, domain, lprim, ∆lprim, true) .≈ ([2,2], [1/∆lprim[2], 0]))
    @test all(MaxwellWave.distweights(c, PRIM, domain, lprim, ∆lprim, false) .≈ ([2,2], [1/∆lprim[2], 0]))

    c = 1  # within range affected by negative-end boundary
    r = (3-c) / (3-0)
    @test all(MaxwellWave.distweights(c, PRIM, domain, lprim, ∆lprim, true) .≈ ([1,2], [1/∆lprim[1]*r, 1/∆lprim[2]*(1-r)]))
    @test all(MaxwellWave.distweights(c, PRIM, domain, lprim, ∆lprim, false) .≈ ([2,2], [1/∆lprim[2]*(1-r), 0]))

    c = 0  # negative-end boundary
    @test all(MaxwellWave.distweights(c, PRIM, domain, lprim, ∆lprim, true) .≈ ([1,1], [1/∆lprim[1], 0]))
    @test all(MaxwellWave.distweights(c, PRIM, domain, lprim, ∆lprim, false) .≈ ([2,2], [0, 0]))

    c = -0.5  # within range affected by negative-end PDC, but outside Bloch and PPC domain (starting at l = 0)
    @test_throws ArgumentError MaxwellWave.distweights(c, PRIM, domain, lprim, ∆lprim, true)
    @test_throws ArgumentError MaxwellWave.distweights(c, PRIM, domain, lprim, ∆lprim, false)
    ...
    @test all(MaxwellWave.distweights(c, PRIM, domain, lprim, ∆lprim, true) .≈ ([10,10], [1/∆lprim[10], 0]))
    @test all(MaxwellWave.distweights(c, PRIM, domain, lprim, ∆lprim, false) .≈ ([10,10], [1/∆lprim[10], 0]))

    c = 105  # within range affected by positive-end boundary
    r = (c-99) / (120-99)
    @test all(MaxwellWave.distweights(c, PRIM, domain, lprim, ∆lprim, true) .≈ ([10,1], [1/∆lprim[10]*(1-r), 1/∆lprim[1]*r]))
    @test all(MaxwellWave.distweights(c, PRIM, domain, lprim, ∆lprim, false) .≈ ([10,10], [1/∆lprim[10]*(1-r), 0]))
    ...
    c = 11  # at grid point
    @test all(MaxwellWave.distweights(c, DUAL, domain, ldual, ∆ldual, true) .≈ ([3,3], [1/∆ldual[3], 0]))
    @test all(MaxwellWave.distweights(c, DUAL, domain, ldual, ∆ldual, false) .≈ ([3,3], [1/∆ldual[3], 0]))

    c = 15  # between grid points
    r = (c-11) / (19-11)
    @test all(MaxwellWave.distweights(c, DUAL, domain, ldual, ∆ldual, true) .≈ ([3,4], [1/∆ldual[3]*(1-r), 1/∆ldual[4]*r]))
    @test all(MaxwellWave.distweights(c, DUAL, domain, ldual, ∆ldual, false) .≈ ([3,4], [1/∆ldual[3]*(1-r), 1/∆ldual[4]*r]))

    c = 1  # farthest point affected by negative-end boundary
    @test all(MaxwellWave.distweights(c, DUAL, domain, ldual, ∆ldual, true) .≈ ([1,1], [1/∆ldual[1], 0]))
    @test all(MaxwellWave.distweights(c, DUAL, domain, ldual, ∆ldual, false) .≈ ([1,1], [1/∆ldual[1], 0]))

    c = 0.5  # within range affected by negative-end boundary
    r = (1-c) / ((1-0) + (120-109))
    @test all(MaxwellWave.distweights(c, DUAL, domain, ldual, ∆ldual, true) .≈ ([1,10], [1/∆ldual[1]*(1-r), 1/∆ldual[10]*r]))
    @test all(MaxwellWave.distweights(c, DUAL, domain, ldual, ∆ldual, false) .≈ ([1,1], [1/∆ldual[1], 0]))
    ...
    @test_throws ArgumentError MaxwellWave.distweights(c, DUAL, domain, ldual, ∆ldual, true)
    @test_throws ArgumentError MaxwellWave.distweights(c, DUAL, domain, ldual, ∆ldual, false)

    c = 109  # farthest point affected by positive-end boundary
    @test all(MaxwellWave.distweights(c, DUAL, domain, ldual, ∆ldual, true) .≈ ([10,10], [1/∆ldual[10], 0]))
    @test all(MaxwellWave.distweights(c, DUAL, domain, ldual, ∆ldual, false) .≈ ([10,10], [1/∆ldual[10], 0]))

    c = 115  # within range affected by positive-end boundary
    r = (c-109) / ((120-109) + (1-0))
    @test all(MaxwellWave.distweights(c, DUAL, domain, ldual, ∆ldual, true) .≈ ([10,1], [1/∆ldual[10]*(1-r), 1/∆ldual[1]*r]))
    @test all(MaxwellWave.distweights(c, DUAL, domain, ldual, ∆ldual, false) .≈ ([10,10], [1/∆ldual[10], 0]))
    ...
    ind_c₁, wt_c₁ = MaxwellWave.distweights(c₁, PRIM, domain, lprim, ∆lprim, true)
    ind_c₂, wt_c₂ = MaxwellWave.distweights(c₂, PRIM, domain, lprim, ∆lprim, true)
    @test ind_c₁ == ind_c₂ == [1,1]
    @test sum(wt_c₁) ≈ sum(wt_c₂) ≈ 1 / ∆lprim[1]

    ind_c₁, wt_c₁ = MaxwellWave.distweights(c₁, DUAL, domain, ldual, ∆ldual, true)
    ind_c₂, wt_c₂ = MaxwellWave.distweights(c₂, DUAL, domain, ldual, ∆ldual, true)
    @test ind_c₁ == ind_c₂ == [1,1]
